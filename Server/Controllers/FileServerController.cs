...
public class FileServerController : Controller 
{
    ...
    private bool GetIsFileServiceEnabled()
    {
        ...
        [HttpPost("list-files")]
        public IActionResult ListFiles([FromBody] ListFilesRequest request)
        {
            if (ValidateRequest(out string message) == false)
                return StatusCode(503, message?.EmptyAsNull() ?? "File service is currently disabled.");
            var result = new LocalFileService().GetFiles(request.Path, request.Path, request.Recursive);
            if(result.IsFailed)
    ...
    public async Task<IActionResult> FileDownload([FromBody]PathRequest path)
    {
        ...
        try
        {
            ...
            if (Request.Headers.ContainsKey("Range") && RangeHeaderValue.TryParse(Request.Headers["Range"], out var rangeHeader))
            {
                ...
                using (var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, BufferSize, FileOptions.Asynchronous))
                {
    ...
    public async Task<IActionResult> Save([FromQuery] string path, [FromQuery] string hash)
    {
        ...
        try
        {
            ...
            outFileInfo.MoveTo(outFileInfo.FullName[..^4], true); // remove the .tmp
    ...
    /// </summary>
    public class ListFilesRequest : PathRequest
    {
